...
function onEvent(funcTable)
	if funcTable[1] == "EX015-saveLoc" then saveLoc(funcTable[2], funcTable[4], funcTable[1]) end
	if funcTable[1] == "EX015-teleportSelf" then teleportSelf(funcTable[2], funcTable[4], funcTable[1]) end
	if funcTable[1] == "EX015-teleportTarget" and funcTable[2]:getEventName() == "EntityDamageByEntityEvent" then teleportTarget(funcTable[2], funcTable[4], funcTable[1]) end
end
...
function saveLoc(event, ability, id)
...
function teleportSelf(event, ability, id)
	if event:getAction():toString() == "RIGHT_CLICK_AIR" or event:getAction():toString() == "RIGHT_CLICK_BLOCK" then
		if event:getPlayer():getInventory():getItemInMainHand() ~= nil then
			if game.isAbilityItem(event:getPlayer():getInventory():getItemInMainHand(), "IRON_INGOT") then
				if game.checkCooldown(game.getPlayer(event:getPlayer()), ability, id) then
					game.checkCooldown(game.getPlayer(event:getPlayer()), ability, "EX015-teleportTarget", false)
...
function teleportTarget(event, ability, id)
	if event:getDamager():getType():toString() == "PLAYER" and event:getEntity():getType():toString() == "PLAYER" then
		...
		if game.isAbilityItem(item, "IRON_INGOT") then
			if game.checkCooldown(game.getPlayer(event:getDamager()), ability, id) then
				game.checkCooldown(game.getPlayer(event:getDamager()), ability, "EX015-teleportSelf", false)
...
function teleport(player, target, ability)
	...
	if targetLoc ~= nil then
		target:getWorld():spawnParticle(import("$.Particle").PORTAL, target:getLocation():add(0,1,0), 1000, 0.1, 0.1, 0.1, 1)
		target:getWorld():playSound(target:getLocation(), import("$.Sound").ITEM_CHORUS_FRUIT_TELEPORT, 0.5, 1)
		target:teleport(targetLoc)
		target:getWorld():spawnParticle(import("$.Particle").REVERSE_PORTAL, target:getLocation():add(0,1,0), 1000, 0.1, 0.1, 0.1, 1)
		target:getWorld():playSound(target:getLocation(), import("$.Sound").ITEM_CHORUS_FRUIT_TELEPORT, 0.5, 1)
