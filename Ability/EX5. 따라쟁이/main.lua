...
function removeAbility(player, id)
	game.removeAbilityAsID(player, id, false)
			...
			if game.checkCooldown(LAPlayer, game.getPlayer(event:getDamager()), ability, id) then
				if game.targetPlayer(LAPlayer, game.getPlayer(event:getEntity())) then
					...
					if #ability > 0 then
						...
						util.executeCommand("la ability " .. ability[1].abilityID, 1, event:getDamager())
						...
						game.addAbility(game.getPlayer(event:getDamager()), ability[1].abilityID, false)
