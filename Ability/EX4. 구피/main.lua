...
function onTimer(player, ability)
	...
	if player:getVariable("EX004-isResurrection") == true then game.sendActionBarMessage(player:getPlayer(), "EX004", "§6[데미지 무시] §a목숨 §7: §2" .. player:getVariable("EX004-lives") .. "개") 
	else game.sendActionBarMessage(player:getPlayer(), "EX004", "§a목숨 §7: §2" .. player:getVariable("EX004-lives") .. "개") end
end

function Reset(player, ability)
	game.sendActionBarMessageToAll("EX004", "")
...
function checkLives(LAPlayer, event, ability, id)
	if event:getEntity():getType():toString() == "PLAYER" then
		if game.checkCooldown(LAPlayer, game.getPlayer(event:getEntity()), ability, id) then
			...
			elseif event:getEntity():getHealth() - event:getFinalDamage() <= 0 then
