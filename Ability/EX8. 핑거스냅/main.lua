...
function onEvent(funcTable)
	if funcTable[1] == "EX008-half" then half(funcTable[2], funcTable[4], funcTable[1]) end
end

function onTimer(player, ability)
	if player:getVariable("EX008-cooldown") == nil then 
		player:setVariable("EX008-cooldown", 0) 
		game.checkCooldown(player, ability, "EX008-half", false)
	end
end

function half(event, ability, id)
