...
function onTimer(player, ability)
	...
	if count > 0 and targetPlayer then
		...
		game.sendActionBarMessage(player:getPlayer(), "§7타겟 : §8" .. targetPlayer:getPlayer():getName() .. " (발동까지 " .. tostring(math.floor(count / 20 + 0.5)) .. "초)")
		game.sendActionBarMessage(targetPlayer:getPlayer(), "§7데스노트 발동까지 " .. tostring(math.floor(count / 20 + 0.5)) .. "초")
	end
		...
		for i = 1, #players do
			if players[i]:getPlayer():getName() == playerName then
				if game.targetPlayer(LAPlayer, players[i]) then
					...
					LAPlayer:setVariable("EX018-count", 2000)
					LAPlayer:setVariable("EX018-isUsed", true)
					
					game.sendMessage(event:getPlayer(), "§8[§7데스노트§8] " .. targetPlayer:getPlayer():getName() .. "§7님은 100초 후 사망합니다.")
					game.sendMessage(event:getPlayer(), "§8[§7데스노트§8] §7만약 당신이 100초 이내에 탈락할 경우, 해당 플레이어는 사망하지 않습니다.")
					
					game.sendMessage(targetPlayer:getPlayer(), "§8[§7데스노트§8] §7당신은 100초 후 데스노트에 의해 사망합니다.")
					return 0 
				else
					return 0 
				end
...
function killPlayer(player)
	...
	if targetPlayer:hasAbility("LA-EX-007") then 
		...
		game.removeAbilityAsID(players[i], "LA-EX-007")
		targetPlayer = player 
	end
	...
	game.removeAbilityAsID(player, "LA-EX-018")
